...
import { Edit3, ShoppingBag, ArrowLeft, Save, AlertCircle } from 'lucide-react';
...
export const EditProduct = () => {
  ...
  useEffect(() => {
    ...
    if (product) {
      setFormData({
        ...
        reorderPoint: product.reorderPoint?.toString() || '15'
      });
    } else {
      addToast(`Product with ID ${id} not found in catalog`, 'error');
      navigate('/admin/inventory');
    }
  }, [id, products, navigate, addToast]);
  ...
  const handleSubmit = (e) => {
    ...
    try {
      ...
      addToast(`SKU ${formData.sku.toUpperCase()} updated successfully!`, 'success');
      navigate('/admin/inventory');
    } catch (err) {
    ...
    return (
      <div className="flex items-center justify-center py-20 text-zinc-500 font-mono text-xs">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="font-bold tracking-widest text-zinc-400 uppercase">Fetching SKU Records...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={handleChange}
                className="w-full text-xs font-semibold px-3 py-2.5 rounded-xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-850 dark:text-zinc-300 focus:outline-none focus:border-amber-500 cursor-pointer"
              >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2 border-t border-zinc-100 dark:border-zinc-850/60">
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                value={formData.gst}
                onChange={handleChange}
                className="w-full text-xs font-semibold px-3 py-2.5 rounded-xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-850 dark:text-zinc-350 focus:outline-none focus:border-amber-500 cursor-pointer"
              >
